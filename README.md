...
- **Rate limiting:** unauthenticated/invalid-key requests are aggressively throttled per client IP; authenticated callers are unlimited.
...
| `RATE_LIMIT_WINDOW_SECONDS`    | `60`                | Rate-limit window length (seconds, unauth only).                       |
| `UNAUTH_RATE_LIMIT_REQUESTS`   | `10`                | Unauthenticated requests per window per IP. Authenticated callers are not rate limited. |
...
| `429`  | `rate_limited`     | Unauthenticated rate limit exceeded (includes `Retry-After`). |
| `503`  | `db_unavailable`   | Database unavailable.                         |

Authenticated callers (valid API key) are **not** rate limited. Unauthenticated
or invalid-key requests are aggressively throttled per client IP
(`UNAUTH_RATE_LIMIT_REQUESTS` per `RATE_LIMIT_WINDOW_SECONDS`); once over the
limit they receive `429` with a `Retry-After` header instead of `401`.
